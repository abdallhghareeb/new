...
class ApiHandle {
  ...
  Future<void> init() async {
    ...
    dio = Dio(BaseOptions(
      ...
      headers: {"lang": lang ?? "ar", 'Content-Type': 'application/json'},
    ));
    /* await Future.wait([
  ...
  Future<Either<DioException, Response>> get(path,
    ...
    try {
      ...
      // if(AuthProvider.isLogin()&&authProvider.userEntity!=null){
      //   bool hasExpired = JwtDecoder.isExpired(authProvider.userEntity!.token!);
      //   if(hasExpired){
      //     authProvider.loginButton(fromSplash: true,fromJWT: true);
      //   }
      // }
      ...
      return Left(
        DioException(
            requestOptions:
                RequestOptions(baseUrl: Constants.baseUri, path: path),
  ...
  Future<Either<DioException, Response>> post(
      path, Map<String, dynamic> data) async {
    print(path);
    print(data);
    try {
      // await reLogin(path);
